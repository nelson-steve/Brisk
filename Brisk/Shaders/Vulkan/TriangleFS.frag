...
layout(set = 1, binding = 10) uniform sampler2D global_textures[];      // 2D textures
layout(set = 1, binding = 10) uniform sampler3D global_textures_3d[];   // 3D textures
...
void main()
{
    ...
    float roughness = texture(global_textures[nonuniformEXT(pushConstants.metallicRoughnessIndex)], inUV0).r;
    float ao = texture(global_textures[nonuniformEXT(pushConstants.occlusionIndex)], inUV0).r;
