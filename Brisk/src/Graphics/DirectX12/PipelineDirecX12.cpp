...
namespace Brisk
{
	void PipelineDirectX12::Init(const GraphicsPipelineSpecs& specs) {
        ...
        std::static_pointer_cast<GpuAdapterDirectX12>(Application::GetGpuAdapter())->GetDevice()->CreateRootSignature(0, serialized->GetBufferPointer(),
        ...
        hr = std::static_pointer_cast<GpuAdapterDirectX12>(Application::GetGpuAdapter())->GetDevice()->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&m_PipelineState));
        if (FAILED(hr)) {
	...
	void PipelineDirectX12::Init(const ComputePipelineSpecs& specs) {
        ...
        if (FAILED(std::static_pointer_cast<GpuAdapterDirectX12>(Application::GetGpuAdapter())->GetDevice()->CreateRootSignature(0, serializedRootSig->GetBufferPointer(),
        ...
        if (FAILED(std::static_pointer_cast<GpuAdapterDirectX12>(Application::GetGpuAdapter())->GetDevice()->CreateComputePipelineState(&computePsoDesc, IID_PPV_ARGS(&m_PipelineState)))) {
            throw std::runtime_error("Failed to create pipeline state");
        }
	}
	...
	void PipelineDirectX12::Bind(std::shared_ptr<CommandBuffer> cmd) {
        ...
        { 
            std::static_pointer_cast<GpuAdapterDirectX12>(Application::GetGpuAdapter())->GetCbvSrvUavHeap().Get(),
            std::static_pointer_cast<GpuAdapterDirectX12>(Application::GetGpuAdapter())->GetSamplerHeap().Get(),
        };
