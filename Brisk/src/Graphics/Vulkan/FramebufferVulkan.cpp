...
namespace Brisk {
	void FramebufferVulkan::Create() {
        ...
        for (size_t i = 0; i < m_SwapchainFramebuffers.size(); i++) {
            ...
            framebufferInfo.renderPass = dynamic_cast<GraphicsDeviceVulkan*>(Engine::m_GPUDevice)->m_GraphicsPipeline->GetRenderPass();
            framebufferInfo.attachmentCount = 1;
            framebufferInfo.pAttachments = attachments;
            framebufferInfo.width = dynamic_cast<SwapchainVulkan*>(Engine::m_Swapchain)->GetExtentWidth();
            framebufferInfo.height = dynamic_cast<SwapchainVulkan*>(Engine::m_Swapchain)->GetExtentHeight();
